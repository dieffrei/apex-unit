...
function toCodacyFormat(salesforceCodeCoverageResult, wideCodeCoverage) {
    ...
    _.each(salesforceCodeCoverageResult, (classCoverage) => {
        ...
        var codacyFormatDataItem = {
            filename: 'src/classes/' + classCoverage.ApexClassOrTrigger.Name + '.cls',
...
const calculateCodeCoverage = async() => {
    ...
    fs.writeFile('codacy-coverage.json', JSON.stringify(json), (fs_error) => {

    });

    /*restler.post("https://api.codacy.com/2.0/"
        + codacyUser + "/" + repoName
